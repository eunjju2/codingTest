...
> 아직 이진 트리나 재귀에 약해서 결국 해설을 보고 이해하고 나서 다시 풀었다 ,, 일단 DFS 인지 BFS 인지 아직 판단이 잘 안 서는 것, 또 재귀에 어떤 파라미터를 넘겨야 하는지, 방문 처리나 반환은 어느 조건에 할 것인지 등등 파악이 아직은 잘 안된다. 이진트리 풀이에 익숙해지고 재귀도 여러번 풀어봐야겠다 😭

### 성능 요약

메모리: 37.9 MB, 시간: 5.59 ms
