...
###🌀재풀이
> 어제 푼 '연속 수열의 합'처럼 단순히 투포인터알고리즘을 이용해 풀려고 했으나, left를 어느 조건에 이동해야 할지 감이 안잡혔다. 저장된 배열 속 첫번째 원소가 그 이후에 포함되어있다면 shift하고 left++를 했으나 시간초과가 떠서(당연함 .. 막연히 includes를 사용함..) 결국 힌트를 보게 되었다. Map을 이용해서 풀어야 했던 문제였다니, 이번 기회에 처음으로 Map을 사용해 보았는데 탐색에 정말 좋은 자료구조인 것 같다. 자주 응용해봐야겠다🫨

### 성능 요약

메모리: 40.4 MB, 시간: 18.04 ms
