...
### 🌀재풀이
> 이전의 **입국심사** 문제처럼 이분탐색으로 풀면 되겠다 했는데, idx 값을 어떻게 알아낼 수 있을까?에서 막혔다 .. 그래도 이전에 풀어본 유형이랑 비슷해서 <br>
 먼저 시간을 알아내고 -> 남은 작업량에서 cores를 순환하며 작업할 수 있는 모든 것들을 뺀 다음 -> 또 한번 순환하며 나머지가 0인것(즉 작업할 수 있는 것)을 빼고 최종적으로 남은 작업량이 0이 되었을 때의 idx를 반환하면 됐다. 

### 성능 요약

메모리: 37.4 MB, 시간: 29.48 ms
