...
### 🌀재풀이
> BFS 오랜만에 푸니까 감을 다 잃었다 ... 🫨 BFS로 모든 노드를 다 방문해서 먹을 수 있는 물고기를 담은 다음, 조건에 맞게 정렬하고 갈 수 있는 최적의 1곳을 반환해서 한마리씩 먹어나간다. <br> BFS 함수 내에서 방문, 정렬, 크기 변환 등 모든 것을 해결하려고 했는데 따로 분리해야 했었다.(이렇게 했더니 먹을 수 있는 물고기 개수가 0인 것을 판단하지 못해서 무한 루프를 멈출 수 없었다) 그리고 조건이 많아서 놓친 부분들이 있었는데, 정렬을 거리순>x값>y값 순으로 해야 했던 것과, 크기가 커지면 cnt를 다시 0으로 초기화해야 했던 것을 놓쳤었다. 문제도 더 자세히 살펴보고, 구조 잘 익혀두기 !

### 성능 요약

메모리: 12748 KB, 시간: 208 ms
